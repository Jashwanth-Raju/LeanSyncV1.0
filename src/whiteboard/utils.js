...
export const formatMinutes = (minutes) => {
    if (!Number.isFinite(minutes ?? NaN) || !minutes || minutes <= 0)
        return "0";
    if (minutes < 1)
...
export const parseNumericValue = (input) => {
    if (!input)
        return null;
...
export const decorateEdge = (edge, edgeThemeMap, defaultVariant) => {
    ...
    return {
        ...
        label: `${theme.iconGlyph ?? ""}${theme.iconGlyph ? " " : ""}${theme.label ?? theme.badge ?? "Connection"}`,
        style: {
            stroke: theme.color,

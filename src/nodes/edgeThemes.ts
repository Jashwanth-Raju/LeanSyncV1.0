...
import type { WhiteboardEdge } from "./NodeTypes";
...
export const edgeThemeMap = edgeLibrary.reduce<Record<string, (typeof edgeLibrary)[number]>>((acc, item) => {
  acc[item.key] = item;
  return acc;
}, {});
...
export const getEdgeVariant = (edge: Edge): string => {
  const variant = (edge.data as WhiteboardEdge["data"])?.connectionType;
  return variant && edgeThemeMap[variant] ? variant : DEFAULT_EDGE_VARIANT;
};

export const decorateEdge = (edge: Edge): Edge => {
  const variant = getEdgeVariant(edge);
  const theme = edgeThemeMap[variant];
  const markerType =
    theme.marker === "arrow" ? MarkerType.Arrow : MarkerType.ArrowClosed;

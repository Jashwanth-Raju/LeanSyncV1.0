...
export const computeFlowMetrics = (nodes) => {
    const totals = {
...
const resolveMetricValue = (metrics, path) => {
...
const inferFormat = (path) => {
    if (/minute|average|takt|lead/i.test(path))
        return "duration";
    if (/ratio|percent/i.test(path))
        return "percent";
    if (/total|count|steps/i.test(path))
        return "number";
    return "raw";
};
const formatValue = (value, format = "raw") => {
    if (value == null)
        return "--";
...
const renderTemplate = (template, metrics) => {
    return template.replace(/\\{\\{([^}]+)\\}\\}/g, (_, token) => {
        ...
        return formatValue(metricValue, formatKey ?? inferredFormat);
    });
};
const buildCard = (config, metrics) => {
...
export const buildDashboardCards = (metrics, config) => {
